...
test-cov: ## Run tests with coverage
	uv run pytest tests/ -v --cov=src/mcp_brave_search --cov-report=term-missing
...
run-http: ## Run HTTP server with uvicorn
	uv run uvicorn mcp_brave_search.server:app --host 0.0.0.0 --port 8000
...
bump: ## Bump version across all files (usage: make bump VERSION=0.2.0)
	...
	@sed -i '' 's/^__version__ = ".*"/__version__ = "$(VERSION)"/' src/mcp_brave_search/__init__.py
	@echo "Updated:"
	@echo "  manifest.json:                    $$(jq -r .version manifest.json)"
	@echo "  server.json:                      $$(jq -r .version server.json)"
	@echo "  pyproject.toml:                   $$(grep '^version' pyproject.toml)"
	@echo "  src/mcp_brave_search/__init__.py: $$(grep '__version__' src/mcp_brave_search/__init__.py)"
